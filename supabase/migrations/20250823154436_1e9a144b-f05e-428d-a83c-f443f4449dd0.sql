...
-- Map 4 existing profile IDs to test providers
WITH user_ids AS (
  SELECT id FROM public.profiles
  ORDER BY created_at ASC
  LIMIT 4
),
-- Label them deterministically
mapped AS (
  SELECT 
    (ARRAY['p1','p2','p3','p4'])[ROW_NUMBER() OVER ()] AS label,
    id
  FROM user_ids
)
-- Clean up any previous seed data for these users
...
-- Insert approved provider profiles for these real users
...
SELECT * FROM (
  SELECT 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1'::uuid, (SELECT id FROM mapped WHERE label='p1'),
  ...
  UNION ALL
  SELECT 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa2'::uuid, (SELECT id FROM mapped WHERE label='p1'),
  ...
  UNION ALL
  SELECT 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbb1'::uuid, (SELECT id FROM mapped WHERE label='p2'),
  ...
  UNION ALL
  SELECT 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbb2'::uuid, (SELECT id FROM mapped WHERE label='p2'),
  ...
  UNION ALL
  SELECT 'cccccccc-cccc-cccc-cccc-ccccccccccc1'::uuid, (SELECT id FROM mapped WHERE label='p3'),
  ...
  UNION ALL
  SELECT 'cccccccc-cccc-cccc-cccc-ccccccccccc2'::uuid, (SELECT id FROM mapped WHERE label='p3'),
  ...
  UNION ALL
  SELECT 'dddddddd-dddd-dddd-dddd-ddddddddddd1'::uuid, (SELECT id FROM mapped WHERE label='p4'),
  ...
  UNION ALL
  SELECT 'dddddddd-dddd-dddd-dddd-ddddddddddd2'::uuid, (SELECT id FROM mapped WHERE label='p4'),
